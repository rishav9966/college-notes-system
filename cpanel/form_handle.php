...
require('connect.php');
   ...
   if(isset($_POST['submitcollege'])){
      ...
      $sql_check = " SELECT * FROM COLLEGE_LIST WHERE  faculty='$college' " ;
      $check_result = mysqli_query($conn, $sql_check);
      $resultcount = mysqli_num_rows($check_result);
      if($resultcount>0){
         $msg = "College " .$college. " Exists" ;
         header("Location:welcome.php?msg=$msg");
      }
      else{
         $sql = " INSERT INTO COLLEGE_LIST (faculty) VALUES('$college')" ;
         mysqli_query($conn, $sql);
         $msg = "College " .$college. " Added successfully";
         header("Location:welcome.php?msg=$msg");
      }
   }
   ...
   if(isset($_POST['submitdept'])){
      ...
      $resultcount = mysqli_num_rows($check_result);
      if($resultcount>0){
         $msg = "Dept " .$dept. " exists in ".$college ;
         header("Location:welcome.php?msg=$msg");
      }
      else{
